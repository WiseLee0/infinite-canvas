...
export const TRANSFORMER_EDGE_RESIZE_RADIUS = 5;
export const TRANSFORMER_ANCHOR_RESIZE_RADIUS = 8;
...
export class RenderTransformer extends System {
  ...
  constructor() {
    ...
    this.query(
      (q) =>
        q
          .using(ComputedBounds, Camera, FractionalIndex)
          .read.and.using(
            Canvas,
            GlobalTransform,
            Transformable,
  ...
  private updateRectMask(camera: Entity) {
    ...
    const { status } = camera.read(Transformable);

    // 当 Select 系统正在进行 resize 或 move 时，mask 已经由 Select 直接同步更新，
    // 跳过此处的更新以避免用过时的 ComputedBounds 数据覆盖。
    if (
...
/**
 * Sync the mask's Transform, Rect and anchor positions from the given OBB.
 */
export function syncMaskTransform(camera: Entity, obb: OBB) {
  const { x, y, width, height, rotation, scaleX, scaleY } = obb;
  const { tlAnchor, trAnchor, blAnchor, brAnchor, mask } =
    camera.read(Transformable);

  Object.assign(mask.write(Rect), { x: 0, y: 0, width, height });
  ...
  Object.assign(tlAnchor.write(Circle), { cx: 0, cy: 0 });
  Object.assign(trAnchor.write(Circle), { cx: width, cy: 0 });
  Object.assign(blAnchor.write(Circle), { cx: 0, cy: height });
  Object.assign(brAnchor.write(Circle), { cx: width, cy: height });
...
/**
 * Show or hide the transformer (mask + corner anchors) for the given camera.
 */
export function setTransformerVisibility(camera: Entity, visible: boolean) {
  const { mask, tlAnchor, trAnchor, blAnchor, brAnchor } =
    camera.read(Transformable);
  const value = visible ? 'visible' : 'hidden';
  mask.write(Visibility).value = value;
  tlAnchor.write(Visibility).value = value;
  trAnchor.write(Visibility).value = value;
  blAnchor.write(Visibility).value = value;
  brAnchor.write(Visibility).value = value;
}
...
export function hitTest(api: API, { x, y }: IPointData) {
  ...
  const { selecteds } = camera.read(Transformable);
  const isMultiSelect = selecteds.length > 1;
  ...
  if (!isMultiSelect && minDistanceToEdges <= TRANSFORMER_EDGE_RESIZE_RADIUS) {
